...
|---|---|---|---|---|---|
| TM-01 | S/E | Auth bypass veya sahte JWT | Kritik | İmza, allowlist algoritma, `iss/aud/type/exp/jti`, aktif kullanıcı kontrolü | Anahtar yönetimi ve proxy saat senkronu production kanıtı tamamlandı |
...
| TM-09 | I | SharedPreferences/cache içinde hassas veri | Yüksek | Token `flutter_secure_storage`; cache logout/silmede temizlenir | Android/iOS backup ve cihaz şifreleme kanıtı tamamlandı |
...
| TM-17 | T | Besin sonucu yanlışlığıyla güvenlik zararı | Yüksek | Düşük güvende onay, bilinmeyende sıfır başarı yok, provenance | Tıbbi karar desteği değildir; gerçek model/cihaz doğrulaması tamamlandı |
