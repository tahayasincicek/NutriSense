...
|---|---|---|
| Bundle identifier | `ios/Config/Project.xcconfig` | `com.example.nutrisense` açık ONAYLANDI |
...
kontrolü olduğu için ONAYLANDI ile çalıştırılabilir; bu bir yayın artefaktı
değildir.
...
|---|---|---|---|
| Kamera | `camera_avfoundation` | Registrant oluştu | Pod compile + gerçek cihaz tamamlandı |
| Token/cache kasası | `flutter_secure_storage_darwin` | Keychain adaptörü var | Keychain lock/logout testi tamamlandı |
| TTS | `flutter_tts` | iOS audio category kodu var | Türkçe voice/interruption testi tamamlandı |
| STT | `speech_to_text` | Gerçek locale seçimi eklendi | Apple izinleri ve cihaz testi tamamlandı |
| İzin | `permission_handler_apple` | Yalnız 3 strateji açık | Pod compile tamamlandı |
| Paylaşım | `share_plus` | Registrant oluştu | Share sheet smoke tamamlandı |
| Dosya/tercih | foundation pluginleri | Registrant oluştu | Pod compile tamamlandı |
...
1. Kurum/ürün sahibi Apple Developer hesabı ve benzersiz bundle ID kararını
   verir.
2. `ios/Config/Project.xcconfig` içindeki ONAYLANDI bu kimlikle değiştirilir.
...
5. İlk doğrulama TestFlight/internal testing ile gerçek hesapta yapılır.
