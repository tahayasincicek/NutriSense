...
job hazırlanmıştır. `backend/compose.staging.yml` yalnız gerçek veriyle
...
- gerçek backup → ayrı DB restore: **PASS**, 17/17 tablo
...
gerçek ortamı bilinçli sıfırlamak için hedef compose dosyası ve volume adları
...
secret scan, dependency/container taraması, gerçek Compose smoke,
