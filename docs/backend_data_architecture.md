...
- boş/ONAYLANDI DB parolası;
- kısa, boş veya ONAYLANDI uygulama/JWT/araştırma secret'ı.
...
Mailpit arayüzü `http://127.0.0.1:8025`, API readiness adresi `http://127.0.0.1:8000/health/ready` olur. gerçek diyetisyen kaydı yalnız dev/test ortamında oluşturulabilir:
...
Seed yalnız `sandbox-dietitian@example.com` adresli gerçek referans kaydı oluşturur; bu kayıt parolalı bir giriş hesabı değildir. Varsayılan bildirim izin listesiyle aynı adres değildir; rapor testi yaparken izin listesini ayrıca ayarlayın.
