...
| Login | UI gecikme çalışması | Gerçek çağrı yoktu | Tokenlar UI akışına bağlanmıyordu. |
...
Önce login sonucu alınır; gerçek token terminal geçmişine yazılmamalıdır. Aşağıdaki değer yalnızca ONAYLANDI'dır:
