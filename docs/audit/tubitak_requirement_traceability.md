...
| YN-09 | Android ve iOS desteklenmeli. | Yöntem, s.4 | `android/`; `ios/`; `docs/ios_release_runbook.md` | Kısmi/Çelişkili | iOS kaynakları ve izin yapılandırması var; Xcode build/archive, IPA/TestFlight ve gerçek cihaz kanıtı yok. Eski “iOS tamamlandı” sonucu geçersizdir. | Kaynak hazırlığı ürün desteği sanılabilir. | Mac üzerinde CocoaPods ve codesign'sız build; gerçek bundle/signing kararı sonrası archive ve iPhone kabul testi üret. | macOS CI build’i ve gerçek cihaz VoiceOver kabul kaydı vardır. |
...
| YN-12 | TalkBack ve VoiceOver ile erişilebilirlik doğrulanmalıdır. | Yöntem, s.4 | Android kodu; `ios/`; `docs/voiceover_manual_test_report.md` | Kısmi | Her iki platform için ortak erişilebilirlik kodu ve iOS kaynakları var; TalkBack/VoiceOver gerçek cihaz kanıtı yok, VoiceOver raporu NOT RUN durumunda. | Yöntem taahhüdü tamamlanamaz. | Her platform için ekran okuyucu test matrisini gerçek cihazlarda yürüt. | Kritik görevler iki platformda test edilir; sorun/çözüm kayıtları eklenir. |
...
| BT-03 | Android ve iOS geliştirme için 4.500 TL hizmet alımı belgelenmeli. | Bütçe, s.8 | Sözleşme/fatura/teslim/IPA yok; iOS kaynakları var | Kanıtsız | Kaynak hazırlığı mali harcama veya kabul edilmiş iOS teslimatı kanıtı değildir. | Harcama ve teslimat doğrulanamaz. | Hizmet kapsamı, teslim kriteri ve mali belgeyi arşivle. | Fatura/sözleşme ile teslim edilen Android+iOS artefaktları eşleşir. |
...
| iOS platformu | **Kısmi** | `ios/`, privacy plist'leri, Podfile ve release guard var; Xcode build/archive, `.ipa`, TestFlight ve gerçek iPhone/VoiceOver kanıtı yok. |
